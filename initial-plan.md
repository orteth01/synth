...
- **Testing:** Vitest for unit tests on DSP math; Playwright for a smoke test that loads the app and verifies a note can be triggered.
...
- Two ADSR envelopes: one for the filter cutoff (with bipolar amount knob), one for amplitude. Exponential segments, not linear — linear envelopes sound wrong on a synth like this.
...
Once mono works end to end, add a polyphony mode with up to 8 voices. Voice stealing: oldest note in the release stage first, otherwise oldest note overall.
...
- CC 1 (mod wheel) → LFO-to-pitch amount.
- CC 64 (sustain pedal).
- All-notes-off / panic.
...
Controls are knobs (rotary, click-and-drag vertically to change, double-click to reset to default, shift-drag for fine adjustment). Use SVG, not canvas — better accessibility, easier theming.
...
5. **Ladder filter** with cutoff and resonance knobs. This is the "now it sounds like a Moog" milestone. Spend time here. A/B against reference recordings.
...
- **Mobile Safari.** Web Audio works but the AudioWorklet has had quirks historically. Test early if mobile is in scope; if not, document it as unsupported.
